	...
	return (
		<div className='grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8'>
			{overviewData.map((item, index) => (
				<motion.div
					key={item.name}
					className='bg-white dark:bg-gray-800 text-black dark:text-white bg-opacity-100 dark:bg-opacity-50 backdrop-filter backdrop-blur-lg shadow-lg rounded-xl p-6 border border-gray-700'
				...
				>
					<div className='flex items-center justify-between'>
						<div>
							<h3 className='text-sm font-medium text-gray-400 dark:text-gray-500'>{item.name}</h3>
					...
					>
						...
						<span className='ml-2 text-sm text-gray-400 dark:text-gray-500'>vs last period</span>
					</div>
				</motion.div>
			))}
		</div>
